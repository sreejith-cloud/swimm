...
@Component({
  selector: 'app-modeof-operation',
  templateUrl: './modeof-operation.component.html',
  styleUrls: ['./modeof-operation.component.css']
})
...
applicationStatus: any;
  ...
  //
  jointlyCheckedFlag:boolean=false;
  anyOneOfTheSurvivorCheckedFlag:boolean=false;
  firstHolderFlag:boolean=false;
  communicationSendToAllFlag:boolean=false;
  disableCheckBoxesFlag:boolean=false;
  ...
  saveModeOfOperation(action) {
    this.modalService.confirm({
      nzTitle: '<i>Confirmation</i>',
      nzContent: '<b>Are you sure want to save ?</b>',
      nzOnOk: () => {

        var modeOfOperationValidFlag: boolean = false;
        modeOfOperationValidFlag = this.ValidateModeOfOperationandCommunnicationToBeSendTo()
        if (modeOfOperationValidFlag) {
          if (action == 'savefinalise') {
            modeOfOperationValidFlag = this.ValidateModeOfOperationandCommunnicationToBeSendToProof();
          }
          else {
            modeOfOperationValidFlag = true;
          }
        }
        if (modeOfOperationValidFlag) {
          var proof = [];
          ...
          let ModeofOperation = [];
          ModeofOperation.push({ "ModeofOperation": this.jointlyCheckedFlag && this.anyOneOfTheSurvivorCheckedFlag ===false ? 'jointly' : 'anyOneOfTheSurvivor'});
          ModeofOperation.push({ "Communicationtobesend": this.firstHolderFlag && this.communicationSendToAllFlag ===false ? 'firstHolder' : 'communicationSendToAll'});
          tandcfulldata.push({ "ModeofOperation": ModeofOperation });
  ...
  FillSaveddata() {
    ...
    if(this.dataforaprove[0] && this.dataforaprove[0].ModeofOperation){
      let toCheckModeOFOperation = this.dataforaprove[0].ModeofOperation
      toCheckModeOFOperation ==='jointly' ? this.jointlyCheckedFlag=true : toCheckModeOFOperation === 'anyOneOfTheSurvivor' ? this.anyOneOfTheSurvivorCheckedFlag=true : ''
    }
    if(this.dataforaprove[0] && this.dataforaprove[0].CommunicationtobeSend){
      let toCheckCommunicationtobesend = this.dataforaprove[0].CommunicationtobeSend
      toCheckCommunicationtobesend ==='firstHolder' ? this.firstHolderFlag=true : toCheckCommunicationtobesend === 'communicationSendToAll' ? this.communicationSendToAllFlag=true : ''
    }
  ...
  ValidateModeOfOperationandCommunnicationToBeSendTo() {
    if (this.jointlyCheckedFlag === false && this.anyOneOfTheSurvivorCheckedFlag === false) {
      this.notif.error('Please Select Mode of Operation', '');
      return false;
    }
    else if(this.firstHolderFlag === false && this.communicationSendToAllFlag ===false) {
      this.notif.error('Please Select Communication to be Send to', '');
      return false;
    }
    else {
      return true;
  ...
  ValidateModeOfOperationandCommunnicationToBeSendToProof() {
    var documents = [];
    documents = this.img.reternImagefinalArray();
    if (documents && documents.length > 0) {
      if (!documents.find((item: any) => item.ProofDoc.DocName.substring(0, 22) == 'Mode of operation form')) {
        this.notif.error('Please upload Mode of operation form', '');
        return false;
      }
      return true;
    }
  ...
  disableFields() {
    if ((this.applicationStatus == 'P') || this.applicationStatus == 'T' || this.applicationStatus == 'F' || this.applicationStatus == 'A') {
      this.disableCheckBoxesFlag = true;
    }
  }
