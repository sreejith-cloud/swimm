...
export class AddressComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  currentPermantAddressOBJ: any;
  corresLocalAddressOBJ: any;
  AdditionalcorresLocalAddressOBJ: any;
  jurisdictionsAddressOBJ: any;
  identyProofOBJ: { 'proofOfAddress': any; };
  PANNO: any;
  hide: boolean = true;
  IsOther: any = 'false';
  NRI: boolean;
  ...
  ngOnInit() {
    ...
    this.dataServ.getResultArray({
      ...
      if (response.results) {
        ...
        console.log("nrivaluetest", nrivaluetest);
        console.log("nritest", nritest);
        ...
        if ((nrivaluetest == 'NRE' || nrivaluetest == 'NRO' || nrivaluetest == 'NROCM' || nrivaluetest == 'NON RESIDENT INDIAN (NRI)') || (nritest == 'NRE' || nritest == 'NRO' || nritest == 'NROCM' || nritest == 'NON RESIDENT INDIAN (NRI)')) {
          ...
          this.cmServ.isNRE.subscribe(val => {
            this.NRI = val
            if (this.NRI) {
              this.form.controls.taxOutsideIndia.patchValue(true)
  ...
  showModal() {
    this.filePreiewContent = this.financialDocument.url
    this.filePreiewFilename = this.financialDocument.DocName
    this.filePreiewContentType = this.financialDocument.type
  ...
  selectPermantADD(data) {
    ...
    if (data != null && data != undefined) {
      if (data == true) {
        ...
        this.sub2 = targetData.controls.street.valueChanges.subscribe(item => {
          form.controls.street.setValue(item);


        })

        this.sub3 = targetData.controls.landmark.valueChanges.subscribe(item => {
  ...
  isIdentityProofValid() {
    ...
    if (dummydata.length == 0) {
      this.notif.error('Please choose Proof of Identity', '', { nzDuration: 10000 })
      return false
    }
  ...
  isAddressValid(action) {
    ...
    if (isAddress1Valid) {
      ...
      let isProofAddress1Valid = this.validServ.validate(dummydata1, "Local/Permanant  Proof of Address")
      if (dummydata1[0] && dummydata1[0].Code && dummydata1[0].Code == '10') {
        if (dummydata1[0].proof0.length != 12) {
          this.notif.error('Invalid Aadhaar details', '');
          isProofAddress1Valid = false;
          return false
        }
      }
      ...
      this.currentPermantAddressOBJ = totalData
      data.push(totalData)
      var JSONData = this.utilServ.setJSONArray(data);
      ...
      if (isAddress2Valid) {
        ...
        let isProofAddress2Valid = this.validServ.validate(dummydata2, "Correspondence/Foreign  Proof of Address")
        if (dummydata2[0] && dummydata2[0].Code && dummydata2[0].Code == '10') {
          if (dummydata2[0].proof0.length != 12) {
            this.notif.error('Invalid Aadhaar details', '');
            isProofAddress2Valid = false;
            return false
          }
        }
        ...
        this.corresLocalAddressOBJ = totalData
        data.push(totalData)
        var JSONData = this.utilServ.setJSONArray(data);
        ...
        this.AdditionalcorresLocalAddressOBJ = AddtotalData
        ...
        if (this.NRI && this.isJudisdiction == true) {
          let isAddress4Valid = true
          if (this.IsOther == 'false' && (action == 'save' || action == 'approve')) {
            isAddress4Valid = this.validServ.validateForm(this.form.controls.address4, this.FormControlNames, this.customValidationMsgObj.JurisAddress);
          }
          if (isAddress4Valid) {
            ...
            if (this.IsOther == 'false') {
              let isProofAddress4Valid = this.validServ.validate(dummydata4, "Jurisdiction  Proof of Address")
            ...
            let totalData = ''
            if (this.IsOther == 'false')
              totalData = { ...this.form.controls.address4.value, ...add4pf, ...this.financialDocument, ...obj }
            else
              totalData = { ...this.form.controls.address4.value, ...this.financialDocument, ...obj }
            this.jurisdictionsAddressOBJ = totalData
            data.push(totalData)
            var JSONData = this.utilServ.setJSONArray(data);
